...
Usage:
    ...
    python inference.py  # defaults to data/CaptchaRaw
...
DATA_DIR = Path(__file__).parent.parent.parent / "data" / "CaptchaTest"
...
def main():
    ...
    if len(sys.argv) < 2:
        if DATA_DIR.is_dir():
            paths.extend(sorted(DATA_DIR.glob("*.png")))
        else:
            print(__doc__.strip())
            print(f"\nDefault folder not found: {DATA_DIR}")
            sys.exit(1)
    else:
